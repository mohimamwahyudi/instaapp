  ...
  <body>
   ...
   <?php } ?>
      <section>
        ...
        <?php 
                    $dbc = new PDO ('mysql:host=localhost;dbname=instaapp','root','');
                    $query = $dbc->prepare("SELECT * FROM postingan,teman,users WHERE  postingan.id_user=teman.id_teman AND teman.id =:id AND users.id=teman.id_teman");
                    $query->bindValue(':id', $_SESSION['id']);
			        $query->execute();
